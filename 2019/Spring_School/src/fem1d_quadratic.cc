...
void Poisson::assemble_system()
{
  ...
  for ( int j = 0 ; j < n_elems ; j++ )
  {
    ...
    int i = 2*j; 
    system_matrix[i][i] += 7.0/3.0*h_inv;
    system_matrix[i+1][i+1] += 16.0/3.0*h_inv;
    system_matrix[i+2][i+2] += 7.0/3.0*h_inv;
    
    system_matrix[i+1][i] += -8.0/3.0*h_inv;
    system_matrix[i][i+1] += -8.0/3.0*h_inv;
    system_matrix[i+1][i+2] += -8.0/3.0*h_inv;
    system_matrix[i+2][i+1] += -8.0/3.0*h_inv;

    system_matrix[i+2][i] += 1.0/3.0*h_inv;
    system_matrix[i][i+2] += 1.0/3.0*h_inv;
    //Add the local rhs to the global rhs
    rhs[i]   += -a*h/6.0;
    rhs[i+1] += -4.0*a*h/6.0;
    rhs[i+2] += -a*h/6.0;
  ...
  if ( p == 2 )
  {
    ...
    system_matrix[n_dofs-1][n_dofs-3] = 0.0;
    system_matrix[n_dofs-3][n_dofs-1] = 0.0;
