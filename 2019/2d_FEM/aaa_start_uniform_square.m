...
bc_type = 1;
case_flag=5; % 0 - 4 = poly examples, 5 = sin, 6 free, 7 = exp, 8 = singular, 9= L-shape, 10=cos(y)*sin(x)
% homo Dirichlet: case 4,7,8

basicName=['data/square_case_',num2str(case_flag),'_uniform_h_version'];
if case_flag==8 || case_flag==7
    scaling=1/2; % scaling of domain = scaling*[-1,1]^2
elseif case_flag==10
    scaling=pi/4;
else
...
p     = 1; % p=polynomial degree
gqn_f = 5;

k=8 ;  % nr of mesh refinements
...
for i=1:k
    ...
    % generate mesh
    [element2vertex,element2face,face2vertex,face_bc,face2neighbor,face_normal,vertex_coor,vertex_bc]=MeshGenerator2d_tri(n,scaling,bc_type);
    element2face=check_local_faceNr(element2vertex,element2face,face2vertex);

    if case_flag==8 || case_flag==7
        for j=1:size(vertex_coor,1)
            vertex_coor(j,1:2) = vertex_coor(j,1:2)+[1  1]/2;
        end
    elseif case_flag==10
        for j=1:size(vertex_coor,1)
            vertex_coor(j,1:2) = vertex_coor(j,1:2)+[1  1]*pi/4;
        end
    end
    disp('mesh done')
    ...
    G_u=dirichletdaten_interpolation_u(element2connectivity,element2vertex,face2vertex,face_bc,face2neighbor,vertex_coor,@(x,y)func_u(x,y,case_flag),innerDofs, totaldof,p);
%     G_u=zeros(totaldof,1);
