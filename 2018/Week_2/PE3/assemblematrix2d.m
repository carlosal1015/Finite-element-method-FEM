...
%
%  Parameters of the transport equation
epsilon = 0.0001; % 1
beta_x = 2; % 0;
beta_y = 1; % 0;
...
Slap =  [ -1/3, -1/3, -1/3 ; -1/3,  8/3, -1/3 ;  -1/3, -1/3, -1/3]; % Laplace Stencil
St_x =h*[ -1/12, 0, 1/12 ; -1/3, 0, 1/3 ; -1/12, 0, 1/12];          % Transport-X Stencil
St_y =h*[ -1/12, -1/3, -1/12; 0, 0, 0; 1/12, 1/3, 1/12];            % Transport-Y Stencil 
SUPG =[-11/6, 2/3, 1/6; -7/3, 20/3, -7/3; 1/6, 2/3, -11/6];         % SUPG-Stencil % beta = (2,1 )

% Stencil of the Matrix
%S = epsilon*Slap +beta_x*St_x + beta_y * St_y;

%S = (epsilon + 2 * 0.5 *h) * Slap +beta_x*St_x + beta_y * St_y; % Ejercicio 3

S = epsilon * Slap + beta_x * St_x + beta_y * St_y;

% stabilization
beta = max(abs(beta_x), abs(beta_y));
delta = h/beta;
% Then,
S = S + delta * SUPG;
